...
int main() {
  auto edit = make_tool_diff(
      "edit", nlohmann::json{{"old_text", "one\ntwo"},
                              {"new_text", "one\nchanged"}},
  ...
  auto create = make_tool_diff("write", nlohmann::json{{"content", "new file\n"}},
                               true, "OK — wrote new.txt");
  if (!create.changed || !create.created ||
      create.body != "+ 1 | new file\n") {
    std::cerr << "write hunk was not rendered like nimlet\n";
    return 1;
  }

  auto numbered = make_tool_diff(
      "edit", nlohmann::json{{"old_text", "old"}, {"new_text", "new"}},
      false, "OK — edited file.cpp\nlines: 12-12 > 12-12");
  ...
  auto replacements = make_tool_diff(
      "edit", nlohmann::json{{"replacements", nlohmann::json::array({
                                  nlohmann::json{{"old_text", "a"},
                                                 {"new_text", "A"}},
                                  nlohmann::json{{"old_text", "b"},
                                                 {"new_text", "B"}}})}},
