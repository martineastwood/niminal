...
int main() {
  ...
  if (fence.find("CODE cpp") == std::string::npos ||
      fence.find("  int x;") == std::string::npos)
  ...
  if (table.find("TABLE 2") == std::string::npos) return fail("table", table);

  auto quote = markdown_outline("> note");
  if (quote.find("Q note") == std::string::npos) return fail("quote", quote);

  auto nested = markdown_outline("```markdown\n# Header 1\n- Item 1\n```");
  if (nested.find("CODE") != std::string::npos) return fail("md fence should render", nested);
