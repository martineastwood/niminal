...
void add_identity(nlohmann::json& out, const niminal::StreamEvent& event) {
  if (!event.session_id.empty()) out["session_id"] = event.session_id;
  if (!event.turn_id.empty()) out["turn_id"] = event.turn_id;
  if (!event.run_id.empty()) out["run_id"] = event.run_id;
}
...
nlohmann::json json_event(const niminal::StreamEvent& event) {
  ...
  switch (event.kind) {
    case niminal::EventKind::run_start:
      out["type"] = "run_start";
      if (!event.text.empty()) out["prompt"] = event.text;
      break;
    case niminal::EventKind::step_start:
      out["type"] = "step_start";
      out["step"] = event.step;
      if (!event.model.empty()) out["model"] = event.model;
      break;
    ...
    case niminal::EventKind::assistant_message:
      return message_event(event.session_id, event.turn_id, "assistant", event.text,
                           event.model, event.final);
  ...
  if (event.duration_ms > 0) out["duration_ms"] = event.duration_ms;
  return out;
}
...
nlohmann::json session_event(const std::string& type, const std::string& session_id,
                             bool success) {
  nlohmann::json out = {
      {"version", kJsonEventVersion}, {"type", type}, {"session_id", session_id}};
  if (type == "session_end") out["success"] = success;
  return out;
}
...
nlohmann::json message_event(const std::string& session_id,
  ...
  if (!turn_id.empty()) out["turn_id"] = turn_id;
  if (!model.empty()) out["model"] = model;
  if (role == "assistant") out["final"] = final;
  return out;
}
...
nlohmann::json queue_event(const std::string& session_id, const std::string& action,
  ...
  nlohmann::json out = {{"version", kJsonEventVersion},
                        ...
                        {"action", action},
                        {"depth", depth}};
  if (!content.empty()) out["content"] = content;
