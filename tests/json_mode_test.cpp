...
int main() {
  ...
  if (start_json.value("version", 0) != 1 || start_json.value("type", "") != "run_start" ||
  ...
  if (call_json.value("type", "") != "tool_call" || call_json.value("tool_id", "") != "t1" ||
  ...
  if (message.value("type", "") != "message" || !message.value("final", false)) {
