...
ThemeMode theme_from_luminance(int r, int g, int b) {
  return (2126 * r + 7152 * g + 722 * b) / 10000 < 128 ? ThemeMode::dark
                                                        : ThemeMode::light;
}
...
std::optional<std::string> query_terminal_background() {
  ...
  if (::write(fd, kQuery, sizeof(kQuery) - 1) > 0) {
    ...
    while (std::chrono::steady_clock::now() < deadline) {
      ...
      if (n > 0) reply.append(buffer, static_cast<size_t>(n));
      if (reply.find('\a') != std::string::npos ||
          reply.find("\033\\") != std::string::npos)
        break;
    }
  }
...
std::optional<ThemeMode> parse_theme_mode(std::string_view value) {
  for (int i = 0; i < 3; ++i) {
    ...
    if (value.size() != name.size()) continue;
    bool match = true;
    ...
    if (match) return static_cast<ThemeMode>(i);
  }
  return std::nullopt;
}
...
std::optional<ThemeMode> theme_from_osc_reply(std::string_view reply) {
  ...
  for (int i = 0; i < 3; ++i) {
    size_t digits = 0;
    while (digits < body.size() && digits < 4 && is_hex(body[digits])) ++digits;
...
std::optional<ThemeMode> theme_from_colorfgbg(std::string_view value) {
  ...
  if (index > 15) return std::nullopt;
...
ThemeMode detect_terminal_theme() {
  static const ThemeMode detected = [] {
    if (auto reply = query_terminal_background())
      if (auto mode = theme_from_osc_reply(*reply)) return *mode;
    if (const char* env = std::getenv("COLORFGBG"))
      if (auto mode = theme_from_colorfgbg(env)) return *mode;
...
Theme resolve_theme(ThemeMode mode) {
  const bool dark = mode == ThemeMode::dark ||
                    (mode == ThemeMode::automatic &&
                     detect_terminal_theme() == ThemeMode::dark);
  if (dark)
    return Theme{
