...
int main() {
  ...
  if (refused) return fail("connection refused should return an error");
  ...
  if (bad_scheme) return fail("invalid url should return an error");

  auto posted = http.post("http://127.0.0.1:1", {}, "body");
  if (posted) return fail("post to closed port should return an error");
  return 0;
}
