...
int main() {
  try {
    if (normalize_thinking(" High ") != "high") return fail("normalize");
  } catch (...) {
  ...
  if (snap_to_efforts("none", {"high", "xhigh"}) != "")
  ...
  if (thinking_body("openrouter", "flash", "medium")["reasoning"].value(
          "effort", "") != "high")
  ...
  if (!thinking_body("openrouter", "toggle-only", "medium")["reasoning"].value(
          "enabled", false))
  ...
  if (thinking_body("anthropic", "claude-sonnet-4-6", "none")["thinking"].value(
          "type", "") != "disabled")
    return fail("anthropic none");
  if (thinking_body("openai", "gpt-5", "high")["reasoning"].value("effort", "") !=
      "high")
    return fail("openai effort");
  if (thinking_body("opencode", "x", "high").value("reasoning_effort", "") !=
      "high")
