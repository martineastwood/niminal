...
int main() {
  if (catalog_name("opencode") != "opencode-go" ||
      catalog_name("opencodezen") != "opencode" ||
  ...
  if (!load_catalog()) return fail("load_catalog");

  auto empty = search_catalog("anthropic", "", 50, {});
  if (empty.size() != 2) return fail("empty query lists provider models");

  auto clau = search_catalog("anthropic", "clau", 50, {});
  if (clau.size() != 2) return fail("substring clau");
  auto sonnet = search_catalog("anthropic", "sonnet", 50, {"claude-sonnet-4-6"});
  if (!sonnet.empty()) return fail("skip recent");
