...
bool glob_match(std::string_view pat, std::string_view str) {
  while (true) {
    if (pat.starts_with("**")) {
      pat.remove_prefix(2);
      if (pat.starts_with('/')) pat.remove_prefix(1);
      if (pat.empty()) return true;
      for (size_t i = 0; i <= str.size(); ++i) {
        if (i != 0 && str[i - 1] != '/') continue;
        if (glob_match(pat, str.substr(i))) return true;
      }
...
bool glob_match_str(const std::string& pat, const std::string& str) {
  if (pat.starts_with("**/")) return glob_match(pat, str);
...
std::string numbered_read(const fs::path& path, int start_line, int end_line) {
  ...
  if (!in) throw WorkspaceError("cannot read " + path.string());
  ...
  if (start_line > n) return "start_line past end of file (" + std::to_string(n) + " lines)\n";
  return out.str();
}

std::string unique_replace(std::string text, const std::string& old_text,
                           const std::string& new_text) {
  if (old_text.empty()) throw WorkspaceError("old_text must not be empty");
  ...
  if (text.find(old_text, pos + 1) != std::string::npos)
    throw WorkspaceError(
        "EDIT_FAILED: old_text matches more than once; add surrounding context");
...
std::string run_bash(const std::string& command, const fs::path& cwd, int timeout_s,
  ...
  int out_pipe[2];
  if (pipe(out_pipe) != 0) throw WorkspaceError(std::strerror(errno));
  ...
  while (true) {
    ...
    if (now >= deadline || (cancel && cancel->load())) {
      timed_out = !(cancel && cancel->load());
      kill(pid, SIGKILL);
      break;
    }
...
wait_child:
  ...
  msg << output;
  if (!output.empty() && output.back() != '\n') msg << '\n';
  if (timed_out) msg << "exit: timeout after " << timeout_s << "s";
  else if (cancel && cancel->load()) msg << "exit: interrupted";
  else msg << "exit: " << code;
  return msg.str();
}
...
void write_file_text(const fs::path& path, const std::string& content) {
  ...
  tmp += ".niminal-tmp";
  {
    ...
    if (!out) throw WorkspaceError("cannot write " + path.string());
    out << content;
  }
  fs::rename(tmp, path);
}
...
std::vector<Tool> workspace_tools(Workspace& ws, std::atomic<bool>* cancel) {
  ...
  tools.push_back(Tool{
      "read",
      "Read a file. Returns numbered lines and a version token.",
      json{{"type", "object"},
           {"properties",
  ...
  tools.push_back(Tool{
      ...
      [&ws](const json& input) {
        ...
        if (pattern.empty()) return std::string("pattern must not be empty");
        ...
        auto grep_file = [&](const fs::path& path, const std::string& rel) {
          ...
          if (ec || size > 1'000'000) return false;
          if (looks_binary(path)) return false;
          std::ifstream in(path);
          std::string line;
          int n = 0;
          while (std::getline(in, line)) {
            ++n;
            if (!std::regex_search(line, rx)) continue;
            hits.push_back(rel + ":" + std::to_string(n) + ":" + line);
            if (static_cast<int>(hits.size()) >= max_hits) return true;
          }
          return false;
        };
        if (!sub.empty()) {
          ...
          if (fs::is_regular_file(start)) {
            ...
            for (const auto& rel : ws.list_files()) {
              if (!path_under(rel, prefix)) continue;
              if (!glob.empty() && !glob_match_str(glob, rel)) continue;
              if (grep_file(ws.resolve(rel), rel)) break;
            }
          }
        } else {
          for (const auto& rel : ws.list_files()) {
        ...
        if (hits.empty()) return std::string("No matches.");
        std::ostringstream out;
  ...
  tools.push_back(Tool{
      "glob",
      "List workspace files matching a glob (e.g. **/*.cpp, src/*).",
      json{{"type", "object"},
           {"properties",
            {{"pattern", {{"type", "string"}}},
             {"path", {{"type", "string"}}}}},
      ...
      [&ws](const json& input) {
        ...
        for (const auto& rel : ws.list_files()) {
          if (!path_under(rel, prefix)) continue;
          if (!glob_match_str(pattern, rel)) continue;
          hits.push_back(rel);
          if (hits.size() >= 200) break;
        }
        if (hits.empty()) return std::string("No files.");
        std::ostringstream out;
        for (size_t i = 0; i < hits.size(); ++i) {
          if (i) out << '\n';
          out << hits[i];
  ...
  tools.push_back(Tool{
      "edit",
      "Replace unique old_text with new_text in a file. Supply expected_version from read.",
      json{{"type", "object"},
           {"properties",
            {{"path", {{"type", "string"}}},
      ...
      [&ws](const json& input) {
        ...
        auto next = unique_replace(read_file_text(path),
                                   input.at("old_text").get<std::string>(),
                                   input.at("new_text").get<std::string>());
        write_file_text(path, next);
        ws.invalidate_listing();
        return "OK — edited " + ws.relative(path) + "\nversion: " +
               ws.file_version(path);
      }});

  tools.push_back(Tool{
      "write",
      "Create a new file or overwrite an existing file. Use edit for targeted changes.",
      json{{"type", "object"},
           {"properties",
            {{"path", {{"type", "string"}}},
             {"content", {{"type", "string"}}},
