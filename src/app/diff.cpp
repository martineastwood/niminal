...
HunkSpan parse_span(std::string_view text) {
  ...
  return {parse_number(old_range.substr(0, old_dash)), parse_number(new_range.substr(0, new_dash))};
}
...
FileDiff make_tool_diff(std::string_view tool_name, const nlohmann::json& input, bool created,
  ...
  if (tool_name == "edit") {
    ...
    if (replacements != input.end() && replacements->is_array() && !replacements->empty()) {
      for (size_t i = 0; i < replacements->size(); ++i) {
        ...
        add_pair(entries, value(replacement, "old_text"), value(replacement, "new_text"), span);
      }
    } else {
  ...
  for (const auto& entry : entries) {
    if (entry.number > 0) {
      width = std::max(width, static_cast<int>(std::to_string(entry.number).size()));
    }
  }
